...
// RetroreflectiveMaterial Definition
class RetroreflectiveMaterial {
    // Adapted from conductorMaterial
  public:
    using BxDF = RetroreflectiveBxDF;
    using BSSRDF = void;

    // RetroreflectiveMaterial Public Methods
    template <typename TextureEvaluator>
    PBRT_CPU_GPU bool CanEvaluateTextures(TextureEvaluator texEval) const {
        return texEval.CanEvaluate({uRoughness, vRoughness}, {eta, k, reflectance});
    }
    ...
    template <typename TextureEvaluator>
    PBRT_CPU_GPU RetroreflectiveBxDF GetBxDF(TextureEvaluator texEval, MaterialEvalContext ctx,
                                       SampledWavelengths &lambda) const {
        // Return BSDF for _RetroreflectiveMaterial_
        Float uRough = texEval(uRoughness, ctx), vRough = texEval(vRoughness, ctx);
        if (remapRoughness) {
            uRough = TrowbridgeReitzDistribution::RoughnessToAlpha(uRough);
            vRough = TrowbridgeReitzDistribution::RoughnessToAlpha(vRough);
        ...
        TrowbridgeReitzDistribution distrib(uRough, vRough);
        return RetroreflectiveBxDF(distrib, etas, ks);
    }

    RetroreflectiveMaterial(SpectrumTexture eta, SpectrumTexture k, SpectrumTexture reflectance,
                      FloatTexture uRoughness, FloatTexture vRoughness,
                      FloatTexture displacement, Image *normalMap, bool remapRoughness)
        : displacement(displacement),
          normalMap(normalMap),
    ...
    static const char *Name() { return "RetroreflectiveMaterial"; }
    ...
    static RetroreflectiveMaterial *Create(const TextureParameterDictionary &parameters,
                                     Image *normalMap, const FileLoc *loc,
                                     Allocator alloc);
