...
pstd::array<Float, 3> SampleSphericalTriangle(const pstd::array<Point3f, 3> &v, Point3f p,
    ...
    Float cosBp;
    if (((SumOfProducts(k2, sinPhi, k1, cosPhi)) * sinAlpha) > 1e-6) {
        cosBp = (k2 + (DifferenceOfProducts(k2, cosPhi, k1, sinPhi)) * cosAlpha) /
                ((SumOfProducts(k2, sinPhi, k1, cosPhi)) * sinAlpha);
    } else {
        cosBp = (k2 + (DifferenceOfProducts(k2, cosPhi, k1, sinPhi)) * cosAlpha) / 1e-6;
    }
