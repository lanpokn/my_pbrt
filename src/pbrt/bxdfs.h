...
// PBRT_CPU_GPU static inline Float ERA(const Float incidentAngle){
...
class RetroreflectiveBxDF {
  public:
    ...
    pstd::optional<BSDFSample> Sample_f(
        ...
        Float R_i = FrDielectric(Dot(wi, wm), 1.59);
        // R = FrDielectric(Dot(wo, wm), 1.59);         
        if (!SameHemisphere(wo, wi))
            return {};
    ...
    SampledSpectrum f(Vector3f wo, Vector3f wi, TransportMode mode) const {
        ...
        if (cosTheta_i == 0 || cosTheta_o == 0)
            return {};
        Vector3f wm = wo + wi;
        Vector3f wm_retro = wo;
        if (LengthSquared(wm) == 0)
            return {};
        wm = Normalize(wm);
        Float R_o = FrDielectric(Dot(wo, wm), 1.59);
        Float R_i = FrDielectric(Dot(wi, wm_retro), 1.59);
        // Evaluate Fresnel factor _F_ for conductor BRDF
        SampledSpectrum F = FrComplex(AbsDot(wo, wm), eta, k);
        SampledSpectrum F_retro = FrComplex(AbsDot(wi, wm_retro), eta, k);
        SampledSpectrum retroreflctive_f = mfDistrib.D(wm_retro) * F_retro * mfDistrib.G(wo, wi)/(4 * cosTheta_i * cosTheta_o);
        return (1- (R_i - R_o))* retroreflctive_f + (1-(R_i - R_o))*(mfDistrib.D(wm) * F * mfDistrib.G(wo, wi) / (4 * cosTheta_i * cosTheta_o));
    }
    ...
    Float PDF(Vector3f wo, Vector3f wi, TransportMode mode,
        ...
        Float R = FrDielectric(Dot(wo, Normal3f(0, 0, 1)), 1.5);
        auto microfacet_pdf = mfDistrib.PDF(wo, wh) / (4 * AbsDot(wo, wh));
