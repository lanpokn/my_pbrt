%为什么这个这么长。。。，看看有没有什么是必须的，没有就只能干苦力了
...
%
% SCENES FROM IMAGE DATA 直接从图片导入场景
...
%   structure assuming the radiance is from a calibrated（校准的标定的） display.  These
...
%   
%     scene = sceneCreate('empty') %建立一个无光子辐射的空场景
%
...
%% Handle the Macbeth parameter cases here %Macbeth 为默认scene
% 
if strncmp(sceneName,'macbeth',5) || ...   %strncmp字符串比较，这里是比较前5个字符，相同返回0
        strcmp(sceneName,'default') || ...
    strcmp(sceneName,'empty')
    patchSize = 16; wave = 400:10:700; surfaceFile = 'macbethChart.mat'; 
    blackBorder = false;
    if ~isempty(varargin), patchSize = varargin{1}; end  % pixels per patch每个色块的边长
    if length(varargin) > 1, wave = varargin{2}; end     % wave
    if length(varargin) > 2, surfaceFile = varargin{3}; end % Reflectances各个色块对不同波长的辐射率
...
scene = sceneInitGeometry(scene); %设置默认distance为1.2米
scene = sceneInitSpatial(scene); %Initialize the scene field of view to 10 deg

useSingle = getpref('ISET', 'useSingle', true);
if useSingle %单精度（4字节）
...
if checkfields(scene,'data','photons') && ~isempty(scene.data.photons)
    ...
    if isempty(sceneGet(scene,'known reflectance')) && checkfields(scene,'data','photons')
        ...
        if isempty(sceneGet(scene,'illuminant')) %自己调试注释掉的
